...
#--------------------------------------------------------------------#
ex_cont20 = ex_cont20 %>% arrange(ind)
for_cont20 = ex_cont20 %>% left_join(udat20,by="ind")
for_cont20 = for_cont20[!is.na(for_cont20$tot_exp),]
temp = for_cont20 %>% select(cntt_name,amt,tot_exp,nn_exp,itm6,itm4,itm2,itm1,buy_tot,buy_spc,memb)
#vis_miss(temp,warn_large_data=F)

str(temp)
temp$memb = ifelse(temp$memb =="비회원",0,1)

registerDoParallel(cores=10)
cont_imp = missForest(data.matrix(temp),parallelize="forests")

arncont20 = cont_imp$ximp %>% as.data.frame 
arncont20 = arncont20 %>% select(cntt_name,amt) %>% cbind("ind"= for_cont20$ind)
arncont20$cntt_name = round(arncont20$cntt_name)
table(arncont20$cntt_name)
table(for_cont20$cntt_name)

arncont20$cntt_name = ifelse(arncont20$cntt_name == 1,"af",arncont20$cntt_name)
arncont20$cntt_name = ifelse(arncont20$cntt_name == 2,"as",arncont20$cntt_name)
arncont20$cntt_name = ifelse(arncont20$cntt_name == 3,"csa",arncont20$cntt_name)
arncont20$cntt_name = ifelse(arncont20$cntt_name == 4,"eu",arncont20$cntt_name)
arncont20$cntt_name = ifelse(arncont20$cntt_name == 5,"me",arncont20$cntt_name)
arncont20$cntt_name = ifelse(arncont20$cntt_name == 6,"na",arncont20$cntt_name)
arncont20$cntt_name = ifelse(arncont20$cntt_name == 7,"oc",arncont20$cntt_name)
table(arncont20$cntt_name)
...
dat20_case2 = tdat20[idx_kotra, ] %>% select(y, 
                                             ...
                                             voc,
                                             bk_num,
                                             bk_num_unq,colnames(contdummies))
dim(dat20_case2) # 125
...
pdf("res/image/ipm_result_age.pdf") 
...
#---------------
pdf("res/image/ipm_result_empl.pdf") 
hist(temp_case2$empl[temp_case2$empl<50],breaks=50,col=col1,main="Imputation result;empl",xlab="empl")
hist(final_case2_imp$empl[final_case2_imp$empl<50],add=TRUE,breaks=50,col=col2)
legend("topright",c("Before","After"),fill=c(col1,col2))
dev.off()
